...
def main():
    ...
    # open_prompt = """
    # Please help me clean up and extract German words with their respective gender articles from the following OCR output to build a study guide for German vocabulary at the {language_level} level.
    # OCR output:
    # {ocr_output}
    # Steps:
    # 1. Clean the OCR output by removing any non-alphabetic characters, punctuation, numbers, non-unicode, and any text that clearly does not represent valid words in any language (e.g., "ae", "ee", "f").
    # 2. Identify and extract all valid German words from the cleaned-up text. Ensure that these words, especially nouns, include their gender-specific articles (der, die, das) and are found in the German dictionary.
    # 3. Exclude any English words or phrases and ensure only valid German words with their articles are included.
    # 4. Convert the extracted German words to lowercase, ensuring the articles remain correct (der, die, das), and remove any duplicates.
    # 5. Use your visual understanding of the image to confirm the extracted German words, their gender articles, and their relevance to the scene.
    # 6. Based on your understanding of the scene and your visual analysis, suggest additional German words and small phrases (nouns with articles, weather conditions, verbs, relationships, adjectives) that are relevant to the image and appropriate for the {language_level} level.
    # 7. Validate the suggested words and phrases to ensure they are all valid German words with correct articles.
    # 8. Assess the quality of the OCR text and assign a quality flag (high, medium, or low) based on the clarity and completeness of the extracted words.
    # 9. Explain the relevance of the suggested words and phrases to the scene and the language level in one sentence, along with an explanation for the quality rating.
    # 10. Provide English translations for each extracted and suggested German word and phrase, ensuring that all translations are accurate and complete.
    # 11. Ensure that only valid German words with articles and phrases are included in the "extracted_words" and "suggested_words" arrays.
    ...
    # Please return the filled-in JSON object as the last part of your response, and precede it immediately with the marker "JSON_START:" Then immediately return the json
    ...
    if args.use_openai:
        ...
        content_responses, model_responses, ocr_texts = batch_ocr_llava_response(prompt, language_level, jpg_files, wait=3, chat=False)
